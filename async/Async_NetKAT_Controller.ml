...
let update_table_for (t : t) (sw_id : switchId) pol : unit Deferred.t =
  ...
  let opt_pol = Optimize.specialize_policy sw_id pol in
  Log.info "%Ld optimizing:\n%s\nto:\n%s" 
    sw_id
    (NetKAT_Pretty.string_of_policy pol)
    (NetKAT_Pretty.string_of_policy opt_pol);
  t.locals <- SwitchMap.add t.locals sw_id opt_pol;
    ...
    let routes = [
      ...
      ("/switch/([1-9][0-9]*)", fun g ->
          ...
          match SwitchMap.find t.locals sw_id with
            ...
            | Some(local) ->
              Server.string_handler (NetKAT_Pretty.string_of_policy local))
    ] in
